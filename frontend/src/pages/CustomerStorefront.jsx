  ...
  return (
    <div className="fixed inset-0 w-full h-full flex flex-col items-center justify-center p-3 sm:p-4 overflow-y-auto bg-slate-100 select-none z-50 text-slate-900">
      ...
      <div className="relative z-10 w-full max-w-[360px] animate-in zoom-in-95 duration-500 my-auto py-1">
        ...
        {/* Top Back Navigation Buttons (Store Selector & Super Admin) */}
        <div className="flex items-center justify-between gap-2 mb-2.5">
          <button
            type="button"
            onClick={() => {
              try {
                localStorage.removeItem('yosafze_selected_shop_id');
                sessionStorage.removeItem('yosafze_selected_shop_id');
              } catch (e) {}
              navigate('/shop');
            }}
            className="inline-flex items-center gap-1.5 text-[9.5px] font-black text-slate-800 hover:text-emerald-700 uppercase tracking-widest bg-white/95 hover:bg-white px-3 py-1.5 rounded-full border border-slate-200 backdrop-blur-md transition-all shadow-sm cursor-pointer active:scale-95"
          >
          ...
          <button
            type="button"
            onClick={() => navigate('/')}
            className="inline-flex items-center gap-1.5 text-[9.5px] font-black text-emerald-800 hover:text-emerald-950 uppercase tracking-widest bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-full border border-emerald-300 backdrop-blur-md transition-all shadow-sm cursor-pointer active:scale-95"
          >
...
function StoreContent({ shopId }) {
  ...
  const handleSaveExpenseSubmit = async (e) => {
    ...
    if (editingExpenseId) {
      ...
      setExpensesList(prev => prev.map(item => String(item._id || item.id) === String(editingExpenseId) ? { ...item, ...updatedExpenseItem } : item));
      try {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.data) {
          setExpensesList(prev => [data.data, ...prev.filter(x => String(x._id || x.id) !== String(data.data._id || data.data.id))]);
        }
        await fetchExpenses();
      } else {
        setExpensesList(prev => [{ ...newExpenseItem, _id: 'exp_' + Date.now() }, ...prev]);
      }
    } catch (err) {
      setExpensesList(prev => [{ ...newExpenseItem, _id: 'exp_' + Date.now() }, ...prev]);
    }
  ...
  const handlePrintSingleExpense = (exp, idx = 0) => {
    ...
    const branchShopId = exp.shopId || shopId || '1';
        ...
        <body>
          <div class="voucher-wrapper">
            <div class="header-banner">
              <div class="header-title">
                <h1>${shopName.toUpperCase()}</h1>
                <p>OFFICIAL EXPENSE PAYMENT VOUCHER &amp; RECEIPT • SHOP ID: ${branchShopId}</p>
              </div>
            ...
            <div class="body-content">
              <div class="info-grid">
                ...
                <div class="info-card">
                  <div class="label">Branch / Shop ID</div>
                  <div class="value">Shop ID: ${branchShopId} (${shopName})</div>
                </div>
                <div class="info-card">
              ...
              <div class="voucher-footer">
                Yosafze Egg Traders • Shop ID: ${branchShopId} • Financial Accounting
              </div>
            </div>
          </div>
        </body>
  ...
  const handleWhatsAppSingleExpense = (exp, idx = 0) => {
    ...
    msg += `📅 *Date:* ${dateStr}\n`;
    msg += `🏢 *Store Branch:* ${shopName} (Shop ID: ${branchShopId})\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    const excelTemplate = `
      ...
      <body>
        <table>
          ...
          <tr>
            <td colspan="4" class="sub-title">OFFICIAL EXPENSE PAYMENT VOUCHER • SHOP ID: ${branchShopId}</td>
          </tr>
          ...
          <tr>
            ...
            <td class="lbl">Shop ID / Branch:</td>
            <td class="val" style="font-weight:bold;">Shop ID: ${branchShopId}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td class="lbl">Paid From:</td>
            <td class="val">${String(exp.paymentSource || exp.paymentMethod || 'CASH').toUpperCase().includes('BANK') ? 'Bank Account' : 'Cash Drawer'}</td>
          </tr>
          <tr>
          ...
          <tr style="height: 12px;"><td colspan="4" style="border:none;"></td></tr>
          <tr>
            <td colspan="4" class="footer-note">Official Expense Voucher • Shop ID: ${branchShopId} • Generated via Yosafze Egg Traders Financial System</td>
          </tr>
        </table>
      </body>
      </html>
                            ...
                            <>
                              ...
                              <div className="block md:hidden space-y-3">
                                {displayedExpenses.map((exp, idx) => {
                                  ...
                                  return (
                                    <div key={`mob_exp_${exp._id || exp.id || idx}`} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all space-y-3">
                                      {/* Card Top: Serial, Shop ID, Title, Category & Amount */}
                                      <div className="flex items-start justify-between gap-2">
                                        <div>
                                          <div className="flex items-center gap-1.5 flex-wrap">
                                            ...
                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-950 border border-emerald-300 rounded text-[9.5px] font-black uppercase shadow-xs">
                                              <Building2 className="w-2.5 h-2.5 text-emerald-800" /> Shop ID: {exp.shopId || shopId}
                                            </span>
                                      ...
                                      {/* Date, Branch & Payment Details */}
                                      <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-150 space-y-1 text-xs">
                                        <div className="flex items-center justify-between text-[11px]">
                                          <span className="text-slate-400 font-bold uppercase text-[9.5px]">Branch Shop ID:</span>
                                          <span className="font-bold text-emerald-900 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 text-[10px]">
                                            Shop ID: {exp.shopId || shopId}
                                          </span>
                                        </div>
                                        <div className="flex items-center justify-between text-[11px]">
                              ...
                              <div className="hidden md:block overflow-x-auto rounded-2xl border border-slate-200 min-h-[350px] pb-24">
                                <table className="w-full text-left text-xs text-slate-800">
                                  <thead className="bg-slate-100 text-[10px] font-black text-slate-600 uppercase tracking-wider border-b border-slate-200">
                                    <tr>
                                      <th className="p-3.5 text-center">#</th>
                                      <th className="p-3.5 text-center">Shop ID</th>
                                  ...
                                  <tbody className="divide-y divide-slate-100">
                                    {displayedExpenses.map((exp, idx) => {
                                      ...
                                      return (
                                        <tr key={exp._id || exp.id || idx} className="hover:bg-slate-50/80 transition-colors">
                                          <td className="p-3.5 text-center font-bold text-slate-400">
                                            {idx + 1}
                                          </td>
                                          <td className="p-3.5 text-center">
                                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-950 border border-emerald-300 shadow-xs">
                                              <Building2 className="w-3 h-3 text-emerald-800" />
                                              Shop ID: {exp.shopId || shopId}
                                            </span>
                                          </td>
...
export function CustomerStorefront() {
  ...
  // If path is /shop (no shopId in URL), ALWAYS show the Branches Selector list (<ShopsList />)!
